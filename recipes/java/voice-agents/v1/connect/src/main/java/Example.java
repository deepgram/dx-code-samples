        ...
        ws.onWelcome(w -> {
            ...
            ws.sendSettings(AgentV1Settings.builder()
                ...
                .agent(AgentV1SettingsAgent.of(AgentV1SettingsAgentContext.builder()
                    ...
                    .greeting("Hello! How can I help?").build()))
                .build());
        });
        ws.onSettingsApplied(s -> { System.out.println("Settings applied"); done.countDown(); });
        ws.onConversationText(t -> System.out.printf("[%s] %s%n", t.getRole(), t.getContent()));
        ws.onError(e -> { System.err.println("Error: " + e.getMessage()); done.countDown(); });

        ws.connect().get(10, TimeUnit.SECONDS);  // <-- THIS: establish WS agent session
